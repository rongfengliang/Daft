...
pub enum AggExpr {
    ...
    #[display("count_distinct({_0}, {_1})")]
    CountDistinct(ExprRef, CountMode),
...
impl AggExpr {
    ...
    pub fn semantic_id(&self, schema: &Schema) -> FieldID {
        match self {
            ...
            Self::CountDistinct(expr, mode) => {
                let child_id = expr.semantic_id(schema);
                FieldID::new(format!("{child_id}.local_count_distinct({mode})"))
            }
            Self::Sum(expr) => {
                let child_id = expr.semantic_id(schema);
    ...
    pub fn with_new_children(&self, mut children: Vec<ExprRef>) -> Self {
        ...
        match self {
            ...
            &Self::CountDistinct(_, count_mode) => Self::CountDistinct(first_child(), count_mode),
...
impl Expr {
    ...
    pub fn count_distinct(self: ExprRef, mode: CountMode) -> ExprRef {
        Self::Agg(AggExpr::CountDistinct(self, mode)).into()
    }
